...
class RamseyXYAnalysis(curve.CurveAnalysis):
        ...
        super().__init__(
            models=[
                lmfit.models.ExpressionModel(
                    expr=f"amp * exp(-x / tau) * cos(2 * pi * ({osc_freq}+freq) * x + phase) + base",
                    name="X",
                    data_sort_key={"series": "X"},
                ),
                lmfit.models.ExpressionModel(
                    expr=f"amp * exp(-x / tau) * sin(2 * pi * ({osc_freq}+freq) * x + phase) + base",
                    name="Y",
    ...
    def _default_options(cls):
        ...
        default_options.result_parameters = [
            curve.ParameterRepr("freq", "\\Delta_ram", "Hz")
        ]
    ...
    def _generate_fit_guesses(
        ...
        decay_data = (data_x.y - user_opt.p0["base"]) ** 2 + (
            data_y.y - user_opt.p0["base"]
        ) ** 2
